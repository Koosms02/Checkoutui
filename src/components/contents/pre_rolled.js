...
const preRolledProduct = (product, addToCart) => {
    ...
    console.log(url)
    return (
        <div className=" bg-white flex flex-row h-[300px]  mt-4 p-2 w-full flex-row rounded">
            {/* Image */}
                            ...
                            {Object.keys(price).map((key) => (
                                price[key] !== undefined && (
                                    <div key={key} className="w-full flex flex-col justify-between items-center">
                                        <div className="flex flex-row w-full justify-between items-end pr-2">
                                            <p className=''>{`${key}: `}</p>
...
export default preRolledProduct;
